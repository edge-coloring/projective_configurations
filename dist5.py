...
# もう少しサイズの計算をキツくしたい。
def sizeDividedByCut(n: int, r: int, G: list, dist: list, cut: list, imply: bool):
    # 連結成分の計算
        ...
        if v >= r:
            ...
            for i in range(num_component):
                if v not in vertices_dist5[i]:
                    if imply:
                        size_component_ring[i] = max(size_component_ring[i], 1 if incident[i] >= 2 else 0)
                    else:
...
def checkContractible(n: int, r: int, G: list, removedG: list, diagonal: dict, u: int, v: int, 
    neighbor_u: int, neighbor_v: int, xu: int, yu: int, xv: int, yv: int, imply: bool):
    ...
    for path in paths:
        s0, s1 = sizeDividedByCut(n, r, G, dist, path + [neighbor_u, neighbor_v], imply)
        ...
        if vertex_u < r and vertex_v < r:
            ...
            for neighbor_vertex_u in vertex_u_neighbors:
                for neighbor_vertex_v in vertex_v_neighbors:
                    ...
                    for path in paths:
                        s0, s1 = sizeDividedByCut(n, r, G, dist, path + [vertex_u, vertex_v], imply)
            ...
            for neighbor_vertex_u in vertex_u_neighbors:
                ...
                for path in paths:
                    s0, s1 = sizeDividedByCut(n, r, G, dist, path + [vertex_u, neighbor_vertex_v], imply)
            ...
            for neighbor_vertex_v in vertex_v_neighbors:
                ...
                for path in paths:
                    s0, s1 = sizeDividedByCut(n, r, G, dist, path + [neighbor_vertex_u, vertex_v], imply)
                    if isForbiddenCut(len(path), s0, s1):
                        return False
        else:
...
# (u, neighvor_v), (neighbor_u, v), (xu, xv), (yu, yv) を対応させる。
# noncontractible に辺 uv が結ばれていると思って、対応する頂点間の距離を計算して、 長さ 5 以下のパスがあれば
# representativity <= 5 に対応しているおり、　non-valid なので False を返す。
def checkNonContractible(n: int, r: int, G: list, removedG: list, diagonal: dict, u: int, v: int, 
    neighbor_u: int, neighbor_v: int, xu: int, yu: int, xv: int, yv: int):
    corresponding_vertices = getCorrespondingVertices(n, r, G, diagonal, u, v, 
        neighbor_u, neighbor_v, xu, yu, xv, yv)
    if not checkNonValidCorrespondingVertices(n, r, corresponding_vertices):
    ...
    # corresponding vertices の間の shortest path のサイズが 5 以下であれば良い。
    for v in range(n):
        if corresponding_vertices[v] == -1:
            continue
        paths = shortestPaths(n, G, v, corresponding_vertices[v])
        if len(paths[0]) <= 6:
            return False
    return True
...
#
# u, v: G 上で距離 5 の頂点対
# imply: bigger imply smaller を仮定してチェックするかどうか。
# 
def checkDist5(n: int, r: int, G: list, removedG: list, u: int, v: int, imply: bool):
    ...
    for i in range(len(u_neighbors)):
        for j in range(len(v_neighbors)):
            ...
            if checkContractible(n, r, G, removedG, diagonal, u, v, u_neighbors[i], v_neighbors[j], 
                diagonal_u[0], diagonal_u[1], diagonal_v[1], diagonal_v[0], imply):
                print(f"contractible dangerous pair: {u} = {v_neighbors[j]}, {u_neighbors[i]} = {v}, {diagonal_u[0]} = {diagonal_v[1]}, {diagonal_u[1]} = {diagonal_v[0]}")
                uv_possible_cnt += 1

            # non-contractible
            if checkNonContractible(n, r, G, removedG, diagonal, u, v, u_neighbors[i], v_neighbors[j], 
                diagonal_u[0], diagonal_u[1], diagonal_v[0], diagonal_v[1]):
                print(f"noncontractible dangerous pair: {u} = {v_neighbors[j]}, {u_neighbors[i]} = {v}, {diagonal_u[0]} = {diagonal_v[0]}, {diagonal_u[1]} = {diagonal_v[1]}")
                uv_possible_cnt += 1
...
def main():
    ...
    parser.add_argument('--imply', action='store_true')
    ...
    for path in sorted(glob.glob(os.path.join(args.dir, "*.conf"))):
        ...
        if args.imply and hasCutVertex(n, r, removedG):
            # カット点を持つ場合は imply の実装をしていないため、チェックする必要がない。
            continue
        dist = WF(n, removedG)
        pairs = getPairsDist5(n, r, dist)
        danger = False
        for u, v in pairs:
            uv_cnt = checkDist5(n, r, G, removedG, u, v, args.imply)
            all_cnt += uv_cnt
